...
userinit(void)
{
  ...
  // 同步程序内存映射到进程内核页表中
  mh_kvm_copy_mappings(p->pagetable, p->proc_kn_pagetable, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    // 内核页表中的映射同步扩大
    if (mh_kvm_copy_mappings(p->pagetable, p->proc_kn_pagetable, sz, n) != 0) {
    ...
    // 内核页表中的映射同步缩小
    sz = mh_kvm_dealloc(p->proc_kn_pagetable, sz, sz + n);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  // 加入调用 kvm_copy_mapping，将新进程用户页表映射拷贝一份到新进程内核页表汇总
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 ||
      mh_kvm_copy_mappings(np->pagetable, np->proc_kn_pagetable, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
