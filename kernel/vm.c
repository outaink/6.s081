...
// 递归打印页表
int
mh_pgtblprint(pagetable_t pagetable, int depth) {
  // 一个页表有 2^9 = 512 个 PTE
  for (int i = 0; i < 512; i++) {
    pte_t pte = pagetable[i];
    ...
    if (pte & PTE_V) {  // 如果页表项有效，按格式打印页表项
      ...
      // 如果不是叶节点，递归打印子节点
      // 作为指向下一级页表的指针，这种页表项不直接映射物理页面，因此不会设置任何权限位（PTE_R、PTE_W、PTE_X）。它只是用来指向一个下一级页表，从而继续进行地址转换。
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0) {
        uint64 child = PTE2PA(pte);
        mh_pgtblprint((pagetable_t)child, depth + 1);
      }
    }
  }
  return 0;
